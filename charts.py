    ...
    try:
        ...
        if chart_t == "bar":
            if safe_hue:
                ...
                # Vertical benchmark line (mean)
                if values and len(values) > 1:
                    try:
                        mean_val = float(np.mean(values))
                        if not np.isnan(mean_val) and mean_val > 0:
                            ax.axvline(mean_val, color="#f59e0b", linestyle=":", linewidth=1.8, alpha=0.85)
                            ax.text(
                                mean_val, n_items - 0.5, f"Avg: {format_num_human(mean_val)}",
                                color="#f59e0b", fontsize=8.5, weight="bold",
                                ha="center", va="bottom",
                                bbox=dict(boxstyle="round,pad=0.25", facecolor=bg_color, edgecolor="#f59e0b", alpha=0.9)
                            )
                    except Exception:
                        pass
